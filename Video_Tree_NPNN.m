classdef Video_Tree_NPNN
    ...
    methods
        ...
        function obj = Video_Tree_NPNN(eta_init, beta_init, gamma, sigmoid_h, lambda, tree_depth, split_prob, node_loss_constant, D, g, n_features, tfpr, max_x, max_y)
        ...
        function obj = train(obj, X_train, coords_train, y_train, X_test, coords_test, y_test, test_repeat)
            ...
            xt_projected_pre = zeros(tree_depth+1, D);
            xt_projected = zeros(tree_depth+1, 2*D);
            ...
            for i=1:n_samples_train
                ...
                for k=1:length(dark_node_indices)
                    ...
                    % make the projection
                    xt_projected_pre(dark_node_index, :) = xt*alpha((dark_node_index-1)*n_features+1:dark_node_index*n_features,:);
                ...
                while isempty(yt_predict_index)
                    ...
                    if isempty(yt_predict_index)
                        ...
                        error('Tree convergence failed...\nbeta_init: %d\ngamma: %.1f\nsigmoid_h: %1.f\nlambda: %3.f\ntree depth: %d\nsplit prob: %.1f\nnode loss constant: %.2fFourier D: %.1f\nFourier g: %.3f', ...
                ...
                for k=length(dark_node_indices):-1:1
                    ...
                    % sgd on each node
                    dz_dw = yt*xt_projected(dark_node_index, :)';
                    dz_db = yt;
                    ...
                    % update random fourier projections
                    result_der = (1/sqrt(D))*(-w(1:D,dark_node_index)'.*sin(xt_projected_pre(dark_node_index, :)) + w(D+1:end,dark_node_index)'.*cos(xt_projected_pre(dark_node_index, :)));
                    alpha((dark_node_index-1)*n_features+1:dark_node_index*n_features,:) = alpha((dark_node_index-1)*n_features+1:dark_node_index*n_features,:) - eta*mu*utility_functions.deriv_sigmoid_loss(z,sigmoid_h)*yt*xt'*result_der;
